...
function Dino({ dinos, filterOption }) {
  ...
  const handleImageError = (dinoId) => {
    setErroredImages(prevErroredImages => [...prevErroredImages, dinoId])
  }
    ...
    <div className="card-container">
      {displayData.map((dino, index) => (
        <div className="card" key={dino.id}>
          ...
          <div className="image">
            <img src={!dino.image ? defaultImage : dino.image} style={{ height: '100px' }} onClick={() => handleSelected(index)} onError={() => handleImageError(dino.id)} />
          </div>
          {selectedDino === index && (
            <div>
              <p>{dino.timePeriod}</p>
